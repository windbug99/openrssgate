    ...
    for row in rows:
        lines.extend(
            [
                ...
                f"  {sql_str(row['description'])}, {sql_str(row['favicon_url'])}, {sql_str(row['language'])}, {sql_str(row['category'])}, {sql_str(row['tags'])},",
                f"  {sql_str(row['status'])}, {sql_str(row['feed_format'])}, {row['fetch_interval_minutes']}, {sql_timestamp(row['last_fetched_at'])}, {sql_timestamp(row['last_published_at'])},",
                f"  {row['consecutive_fail_count']}, {sql_str(row['registered_by'])}, {sql_str(row['status_reason'])}, {sql_str(row['type'])}, {sql_str(row['categories'])}",
                ")",
